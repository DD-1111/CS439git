...
uint32_t Node::rename(const char* fileName, const char* new_filename) {
    ...
    if(res == 1) {
        write_all(off, (char *)new_filename, K::strlen(fileName));
    }

    return res;
}

uint32_t Node::remove(Shared<Node> fileNode, const char* fileName) {
    ...
    if(res == 1) {
        uint32_t start = off - 8;
        uint32_t end = off + K::strlen(fileName);
        //moving following entry one unit forward
        while (end < data.size_low) {
            ...
            Debug::printf("finish inode\n");

            Debug::printf("start : %d\n", start);

            uint32_t total_size;
            read(end,total_size);
            write_all(start, (char *)total_size, 2);
            start += 2;
            end += 2;
            Debug::printf("finish total_size\n");

            Debug::printf("start : %d\n", start);
            
            uint32_t name_length;
            read(end,name_length);
            write_all(start, (char *)name_length, 2);
            start += 2;
            end += 2;
            Debug::printf("finish name_length\n");

            char* name = (char*) malloc(name_length);
            name[name_length] = 0;
            read_all(end, name_length, name);

            Debug::printf("name : %s\n", name);
            write_all(start, name, name_length);
            start += name_length;
            end += name_length;
