...
int64_t BlockIO::write(uint32_t offset, char* buffer, uint32_t n) {
    uint32_t sector = offset / block_size;
    uint32_t start = offset % block_size;

    // Debug::printf("buffer size: %d\n", n);
...
int64_t BlockIO::write_all(uint32_t offset, char* buffer, uint32_t n) {
