...
{   // we implement BlockIO because we
    // represent data
...
public:
    ...
    // we use number to get the blovk and read the whole block to buffer
    void read_block(uint32_t number, char *buffer) override;

    //we use number to get the blovk and write the whole block to buffer
...
public:
    ...
    //abs path of the directory
    /* return value: 1->succesfully remove, -1->no such file*/
    uint32_t removeFile(const char *path, const char *filename)
    {
        Shared<Node> dirNode = find(root, path);
        Shared<Node> fileNode = find(dirNode, filename);
        ASSERT(fileNode->is_file());
        return dirNode->remove(filename);
    }
};
